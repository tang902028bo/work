...
(function (layui, window, factory) {
  ...
  var v = '4.0.0.0910',
    ...
    this.config = {
      ...
      maxTips: function maxTips(id, vals, val, max) {
        var ipt = $("[xid=\"".concat(_this.config.name, "\"]")).prev().find(".".concat(NAME));
  ...
  Common.prototype.init = function (target) {
    ...
    //初始化页面上已有的select
    $(target ? target : "select[".concat(NAME, "]")).each(function (index, select) {
      var othis = $(select),
          id = othis.attr(NAME),
          hasLayuiRender = othis.next(".layui-form-select"),
          hasRender = othis.next(".".concat(PNAME)),
          options = {
        name: id,
      ...
      if (fs.config.init) {
        fs.values = fs.config.init.map(function (item) {
          ...
          return {
            name: othis.find("option[value=\"".concat(item, "\"]")).text(),
            value: item
          };
      ...
      !fs.values && (fs.values = []);
      data[id] = fs; //先取消layui对select的渲染

      hasLayuiRender[0] && hasLayuiRender.remove();
      hasRender[0] && hasRender.remove(); //构造渲染div
      ...
      var inputHtml = ["<div class=\"".concat(LABEL, "\">"), "<input type=\"text\" fsw class=\"".concat(FORM_INPUT, " ").concat(INPUT, "\" ").concat(fs.config.isSearch ? '' : 'style="display: none;"', " autocomplete=\"off\" debounce=\"0\" />"), "</div>"];
      var reElem = $("<div class=\"".concat(FORM_SELECT, "\" ").concat(SKIN, "=\"").concat(fs.config.skin, "\">\n\t\t\t\t\t<input class=\"").concat(HIDE_INPUT, "\" value=\"\" name=\"").concat(fs.config.formname, "\" lay-verify=\"").concat(fs.config.layverify, "\" lay-verType=\"").concat(fs.config.layverType, "\" type=\"text\" style=\"position: absolute;bottom: 0; z-index: -1;width: 100%; height: 100%; border: none; opacity: 0;\"/>\n\t\t\t\t\t<div class=\"").concat(FORM_TITLE, " ").concat(fs.config.disabled ? DIS : '', "\">\n\t\t\t\t\t\t<div class=\"").concat(FORM_INPUT, " ").concat(NAME, "\" ").concat(heightStyle, ">\n\t\t\t\t\t\t\t").concat(inputHtml.join(''), "\n\t\t\t\t\t\t\t<i class=\"").concat(SANJIAO, "\"></i>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div class=\"").concat(TDIV, "\">\n\t\t\t\t\t\t\t<input type=\"text\" autocomplete=\"off\" placeholder=\"").concat(fs.config.placeholder, "\" readonly=\"readonly\" unselectable=\"on\" class=\"").concat(FORM_INPUT, "\">\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div></div>\n\t\t\t\t\t</div>\n\t\t\t\t\t<dl xid=\"").concat(id, "\" class=\"").concat(DL, " ").concat(fs.config.radio ? RADIO : '', "\">").concat(dinfo, "</dl>\n\t\t\t\t</div>"));
      var $parent = $("<div class=\"".concat(PNAME, "\" FS_ID=\"").concat(id, "\"></div>"));
      $parent.append(reElem);
      othis.after($parent);
      othis.attr('lay-ignore', '');
      othis.removeAttr('name') && othis.attr('_name', fs.config.formname);
      othis.removeAttr('lay-verify') && othis.attr('_lay-verify', fs.config.layverify); //如果可搜索, 加上事件

      if (fs.config.isSearch) {
        ajaxs[id] = $.extend({}, ajax, {
          searchUrl: fs.config.searchUrl
        }, ajaxs[id]);
        $(document).on('input', "div.".concat(PNAME, "[FS_ID=\"").concat(id, "\"] .").concat(INPUT), function (e) {
        ...
        //隐藏第二个dl
        reElem.find("dl dd.".concat(FORM_DL_INPUT)).css('display', 'none');
      }
    });
  };
  ...
  Common.prototype.search = function (id, e, searchUrl, call) {
    ...
    var inputValue = $.trim(input.value); //过滤一下tips
    ...
    var fs = data[id],
        ...
        reElem = $("dl[xid=\"".concat(id, "\"]")).parents(".".concat(FORM_SELECT)); //如果开启了远程搜索
    ...
    if (searchUrl) {
      ...
      if (!ajaxConfig.beforeSearch || ajaxConfig.beforeSearch && _instanceof(ajaxConfig.beforeSearch, Function) && ajaxConfig.beforeSearch(id, searchUrl, inputValue)) {
        ...
        fs.clearid = setTimeout(function () {
          reElem.find("dl > *:not(.".concat(FORM_SELECT_TIPS, ")")).remove();
          reElem.find("dd.".concat(FORM_NONE)).addClass(FORM_EMPTY).text('请求中');

          _this3.ajax(id, searchUrl, inputValue, false, null, true);
        }, delay);
      }
    } else {
      reElem.find("dl .".concat(DD_HIDE)).removeClass(DD_HIDE); 
     //遍历选项, 选择可以显示的值
      reElem.find("dl dd:not(.".concat(FORM_SELECT_TIPS, ")")).each(function (idx, item) {
        var _item = $(item);
      ...
      reElem.find('dl dt').each(function (index, item) {
        if (!$(item).nextUntil('dt', ":not(.".concat(DD_HIDE, ")")).length) {
          $(item).addClass(DD_HIDE);
        }
      }); 
      //动态创建
      this.create(id, isCreate, inputValue);
      var shows = reElem.find("dl dd:not(.".concat(FORM_SELECT_TIPS, "):not(.").concat(DD_HIDE, ")"));

      if (!shows.length) {
        reElem.find("dd.".concat(FORM_NONE)).addClass(FORM_EMPTY).text('无匹配项');
      } else {
        reElem.find("dd.".concat(FORM_NONE)).removeClass(FORM_EMPTY);
      }
    }
  };
  ...
  Common.prototype.triggerSearch = function (div, isCall) {
    ...
    (div ? [div] : $(".".concat(FORM_SELECT)).toArray()).forEach(function (reElem, index) {
      reElem = $(reElem);
      var id = reElem.find('dl').attr('xid');

      if (id && data[id] && data[id].config.isEmpty || isCall) {
        _this4.search(id, null, null, data[id].config.searchType == 0 ? reElem.find(".".concat(LABEL, " .").concat(INPUT)) : reElem.find("dl .".concat(FORM_DL_INPUT, " .").concat(INPUT)));
      }
    });
  };

  Common.prototype.clearInput = function (id) {
    var div = $(".".concat(PNAME, "[fs_id=\"").concat(id, "\"]"));
    var input = data[id].config.searchType == 0 ? div.find(".".concat(LABEL, " .").concat(INPUT)) : div.find("dl .".concat(FORM_DL_INPUT, " .").concat(INPUT));
    input.val('');
  };
  ...
  Common.prototype.ajax = function (id, searchUrl, inputValue, isLinkage, linkageWidth, isSearch, successCallback, isReplace) {
    ...
    var reElem = $(".".concat(PNAME, " dl[xid=\"").concat(id, "\"]")).parents(".".concat(FORM_SELECT));
    ...
    ajaxData[ajaxConfig.searchName] = inputValue; //是否需要对ajax添加随机时间
    //ajaxData['_'] = Date.now();
    ...
    $.ajax({
      ...
      success: function success(res) {
        ...
        ajaxConfig.beforeSuccess && _instanceof(ajaxConfig.beforeSuccess, Function) && (res = ajaxConfig.beforeSuccess(id, searchUrl, inputValue, res));
        ...
        if (res[ajaxConfig.response.statusName] != ajaxConfig.response.statusCode) {
          reElem.find("dd.".concat(FORM_NONE)).addClass(FORM_EMPTY).text(res[ajaxConfig.response.msgName]);
        } else {
          reElem.find("dd.".concat(FORM_NONE)).removeClass(FORM_EMPTY);
        ...
        ajaxConfig.success && _instanceof(ajaxConfig.success, Function) && ajaxConfig.success(id, searchUrl, inputValue, res);
      },
      error: function error(err) {
        reElem.find("dd[lay-value]:not(.".concat(FORM_SELECT_TIPS, ")")).remove();
        reElem.find("dd.".concat(FORM_NONE)).addClass(FORM_EMPTY).text('服务异常');
        ajaxConfig.error && _instanceof(ajaxConfig.error, Function) && ajaxConfig.error(id, searchUrl, inputValue, err);
      }
    });
  };
  ...
  Common.prototype.renderData = function (id, dataArr, linkage, linkageWidth, isSearch, isReplace) {
    ...
    var reElem = $(".".concat(PNAME, " dl[xid=\"").concat(id, "\"]")).parents(".".concat(FORM_SELECT));
    var ajaxConfig = ajaxs[id] ? ajaxs[id] : ajax;
    var pcInput = reElem.find(".".concat(TDIV, " input"));
    ...
    this.addTreeHeight(reElem.find('dl').find('dd[tree-id]').first(), reElem.find('dl').find('dd[tree-id]').first().height());
    var label = reElem.find(".".concat(LABEL));
    ...
    if (isSearch) {
      ...
      oldVal.forEach(function (item, index) {
        dl.find("dd[lay-value=\"".concat(item.value, "\"]")).addClass(THIS);
      });
  ...
  Common.prototype.renderLinkage = function (id, dataArr, linkageWidth) {
    ...
    $.each(result, function (idx, arr) {
      var groupDiv = ["<div style=\"left: ".concat((linkageWidth - 0) * idx, "px;\" class=\"xm-select-linkage-group xm-select-linkage-group").concat(idx + 1, " ").concat(idx != 0 ? 'xm-select-linkage-hide' : '', "\">")];
      $.each(arr, function (idx2, item) {
        var span = "<li title=\"".concat(item.name, "\" pid=\"").concat(item.pid, "\" xm-value=\"").concat(item.value, "\"><span>").concat(item.name, "</span></li>");
        groupDiv.push(span);
      });
      groupDiv.push("</div>");
    ...
    reElem.find(".".concat(INPUT)).css('display', 'none'); //联动暂时不支持搜索
  };

  Common.prototype.renderReplace = function (id, dataArr) {
    var _this7 = this;

    var dl = $(".".concat(PNAME, " dl[xid=\"").concat(id, "\"]"));
    var ajaxConfig = ajaxs[id] ? ajaxs[id] : ajax;
    dataArr = this.exchangeData(id, dataArr);
    db[id] = dataArr;
    ...
    dl.find("dd:not(.".concat(FORM_SELECT_TIPS, "),dt:not([style])")).remove();
  ...
  Common.prototype.getChildrenList = function (arr, childrenName, disabledName, pid, disabled) {
    ...
    for (var a = 0; a < arr.length; a++) {
      ...
      parentIds.push("".concat(offset - 1, "_E"));
  ...
  Common.prototype.create = function (id, isCreate, inputValue) {
    if (isCreate && inputValue) {
      var fs = data[id],
          dl = $("[xid=\"".concat(id, "\"]")),
          tips = dl.find("dd.".concat(FORM_SELECT_TIPS, ".").concat(FORM_DL_INPUT)),
          tdd = null,
          temp = dl.find("dd.".concat(TEMP));
      dl.find("dd:not(.".concat(FORM_SELECT_TIPS, "):not(.").concat(TEMP, ")")).each(function (index, item) {
      ...
      if (!tdd) {
        ...
        if (temp[0]) {
          ...
          tips.after($(this.createDD(id, {
            ...
            value: val
          }, "".concat(TEMP, " ").concat(CREATE_LONG))));
        }
      }
    } else {
      $("[xid=".concat(id, "] dd.").concat(TEMP)).remove();
    }
  };
  ...
  Common.prototype.createDD = function (id, item, clz) {
    ...
    var attr = pid[0] == -1 ? '' : "tree-id=\"".concat(pid.join('-'), "\" tree-folder=\"").concat(!!item['XM_TREE_FOLDER'], "\"");
    return "<dd lay-value=\"".concat(item.value, "\" class=\"").concat(item.disabled ? DISABLED : '', " ").concat(clz ? clz : '', " ").concat(item.is_group ? 'group_dd' : '', "\" ").concat(attr, ">\n\t\t\t\t\t<div class=\"xm-unselect xm-form-checkbox ").concat(item.disabled ? DISABLED : '', "\"  style=\"margin-left: ").concat((pid.length - 1) * 30, "px\">\n\t\t\t\t\t\t") + (item.is_group ? '' : '<i class="' + CHECKBOX_YES + '"></i>') + "\n\t\t\t\t\t\t<span name=\"".concat(name, "\">").concat(template, "</span>\n\t\t\t\t\t</div>\n\t\t\t\t</dd>");
  };

  Common.prototype.createQuickBtn = function (obj, right) {
    return "<div class=\"".concat(CZ, "\" method=\"").concat(obj.name, "\" title=\"").concat(obj.name, "\" ").concat(right ? 'style="margin-right: ' + right + '"' : '', "><i class=\"").concat(obj.icon, "\"></i><span>").concat(obj.name, "</span></div>");
  };
  ...
  Common.prototype.renderBtns = function (id, show, right) {
    ...
    var quickBtn = [];
    var dl = $("dl[xid=\"".concat(id, "\"]"));
    quickBtn.push("<div class=\"".concat(CZ_GROUP, "\" show=\"").concat(show, "\" style=\"max-width: ").concat(dl.prev().width() - 54, "px;\">"));
  ...
  Common.prototype.renderSelect = function (id, tips, select) {
    ...
    if (data[id].config.btns.length) {
      setTimeout(function () {
        var dl = $("dl[xid=\"".concat(id, "\"]"));
        dl.parents(".".concat(FORM_SELECT)).attr(SEARCH_TYPE, data[id].config.searchType);
        dl.find(".".concat(CZ_GROUP)).css('max-width', "".concat(dl.prev().width() - 54, "px"));
      }, 10);
      arr.push(["<dd lay-value=\"\" class=\"".concat(FORM_SELECT_TIPS, "\" style=\"background-color: #FFF!important;\">"), this.renderBtns(id, null, '30px'), "</dd>", "<dd lay-value=\"\" class=\"".concat(FORM_SELECT_TIPS, " ").concat(FORM_DL_INPUT, "\" style=\"background-color: #FFF!important;\">"), "<i class=\"xm-iconfont icon-sousuo\"></i>", "<input type=\"text\" class=\"".concat(FORM_INPUT, " ").concat(INPUT, "\" placeholder=\"\u8BF7\u641C\u7D22\"/>"), "</dd>"].join(''));
    } else {
      arr.push("<dd lay-value=\"\" class=\"".concat(FORM_SELECT_TIPS, "\">").concat(tips, "</dd>"));
    }
    ...
    if (this.isArray(select)) {
      $(select).each(function (index, item) {
        if (item) {
          if (item.type && item.type === 'optgroup') {
            arr.push("<dt>".concat(item.name, "</dt>"));
          } else {
      ...
      $(select).find('*').each(function (index, item) {
        ...
        if (item.tagName.toLowerCase() === 'optgroup') {
          arr.push("<dt>".concat(item.label, "</dt>"));
        } else {
          arr.push(_this9.createDD(id, item));
        }
      });
    ...
    arr.push("<dd class=\"".concat(FORM_SELECT_TIPS, " ").concat(FORM_NONE, " ").concat(arr.length === 2 ? FORM_EMPTY : '', "\">\u6CA1\u6709\u9009\u9879</dd>"));
    return arr.join('');
  };
  ...
  Common.prototype.on = function () {
    ...
    $(document).on('click', function (e) {
      if (!$(e.target).parents(".".concat(FORM_TITLE))[0]) {
        //清空input中的值
        $(".".concat(PNAME, " dl .").concat(DD_HIDE)).removeClass(DD_HIDE);
        $(".".concat(PNAME, " dl dd.").concat(FORM_EMPTY)).removeClass(FORM_EMPTY);
        $(".".concat(PNAME, " dl dd.").concat(TEMP)).remove();
        $.each(data, function (key, fs) {
          _this10.clearInput(key);

          if (!fs.values.length) {
            _this10.changePlaceHolder($("div[FS_ID=\"".concat(key, "\"] .").concat(LABEL)));
          }
        });
      }

      $(".".concat(PNAME, " .").concat(FORM_SELECTED)).each(function (index, item) {
        _this10.changeShow($(item).find(".".concat(FORM_TITLE)), false);
      });
    });
  };
  ...
  Common.prototype.one = function (target) {
    ...
    //一次性事件绑定
    $(target ? target : document).off('click', ".".concat(FORM_TITLE)).on('click', ".".concat(FORM_TITLE), function (e) {
      var othis = $(e.target),
          title = othis.is(FORM_TITLE) ? othis : othis.parents(".".concat(FORM_TITLE)),
          dl = title.next(),
          id = dl.attr('xid'); //清空非本select的input val

      $("dl[xid]").not(dl).each(function (index, item) {
        _this11.clearInput($(item).attr('xid'));
      });
      $("dl[xid]").not(dl).find("dd.".concat(DD_HIDE)).removeClass(DD_HIDE); //如果是disabled select
      ...
      if (othis.is(".".concat(SANJIAO)) || othis.is(".".concat(INPUT, "[readonly]"))) {
        _this11.changeShow(title, !title.parents(".".concat(FORM_SELECT)).hasClass(FORM_SELECTED));

        return false;
      } //如果点击的是input的右边, focus一下


      if (title.find(".".concat(INPUT, ":not(readonly)"))[0]) {
        $(document).click();

        var input = title.find(".".concat(INPUT)),
            epos = {
          x: e.pageX,
          y: e.pageY
        },
        ...
        while (epos.x > pos.x) {
          if ($(document.elementFromPoint(epos.x, epos.y)).is(input)) {
            ...
            if(typeof IS_MOBILE==='boolean' && !IS_MOBILE) //手机端不获取焦点
            input.focus();

            _this11.changeShow(title, true); 

            //收缩所有的分组
            // if (dl.children('.group_dd').length) {
            //   dl.children('.group_dd').filter(function () {
            //     return !$(this).children('.icon-expand').length;
            //   }).append('<i class="xm-iconfont icon-expand"></i>');
            //   dl.children(':not(.group_dd)').css('height', 0);
            // }
      ...
      //如果点击的是可搜索的input
      if (othis.is(".".concat(INPUT))) {
        _this11.changeShow(title, true);

        return false;
      } 
      //如果点击的是x按钮
      if (othis.is("i[fsw=\"".concat(NAME, "\"]"))) {
        var val = _this11.getItem(id, othis),
            dd = dl.find("dd[lay-value='".concat(val.value, "']"));
    ...
    $(target ? target : document).off('click', "dl.".concat(DL)).on('click', "dl.".concat(DL), function (e) {
      var othis = $(e.target);

      if (othis.is(".".concat(LINKAGE)) || othis.parents(".".concat(LINKAGE))[0]) {
        //linkage的处理
        othis = othis.is('li') ? othis : othis.parents('li[xm-value]');

        var group = othis.parents('.xm-select-linkage-group'),
        ...
        othis.addClass('xm-select-active'); //激活下一个group, 激活前显示对应数据

        group.nextAll('.xm-select-linkage-group').addClass('xm-select-linkage-hide');
        var nextGroup = group.next('.xm-select-linkage-group');
        nextGroup.find('li').addClass('xm-select-linkage-hide');
        nextGroup.find("li[pid=\"".concat(othis.attr('xm-value'), "\"]")).removeClass('xm-select-linkage-hide'); //如果没有下一个group, 或没有对应的值
        ...
        if (!nextGroup[0] || nextGroup.find("li:not(.xm-select-linkage-hide)").length == 0) {
          ...
          do {
            ...
            othis = othis.parents('.xm-select-linkage-group').prev().find("li[xm-value=\"".concat(othis.attr('pid'), "\"]"));
          } while (othis.length);
      ...
      if (treeId) {
        ...
        var childrens = dd.nextAll("dd[tree-id^=\"".concat(treeId, "\"]"));

        if (childrens && childrens.length) {
          var len = childrens[0].clientHeight;
          len ? (_this11.addTreeHeight(dd, len), len = 0) : (len = dd.attr('xm-tree-hidn') || 36, dd.removeAttr('xm-tree-hidn'), //dd.find('>i').remove(), liucheng20190301 树状结构点击不隐藏图标
          childrens = childrens.filter(function (index, item) {
          ...
          if (treeConfig.nextClick && _instanceof(treeConfig.nextClick, Function)) {
      ...
      if (dd.hasClass(FORM_SELECT_TIPS)) {
        //tips的处理
        var btn = othis.is(".".concat(CZ)) ? othis : othis.parents(".".concat(CZ));
        ...
        obj && obj.click && _instanceof(obj.click, Function) && obj.click(id, _this11);
        return false;
      }
  ...
  Common.prototype.linkageAdd = function (id, val) {
    ...
    do {
      pid = vs[index];
      li = dl.find(".xm-select-linkage-group".concat(index + 1, " li[xm-value=\"").concat(pid, "\"]"));
      li[0] && lis.push(li);
      index++;
  ...
  Common.prototype.linkageDel = function (id, val) {
    ...
    do {
      ...
      if (!li.parent().next().find("li[pid=".concat(pid, "].xm-select-this")).length) {
  ...
  Common.prototype.valToName = function (id, val) {
    ...
    $.each(vs, function (idx, item) {
      var name = dl.find(".xm-select-linkage-group".concat(idx + 1, " li[xm-value=\"").concat(item, "\"] span")).text();
      names.push(name);
    });
  ...
  Common.prototype.commonHandler = function (key, label) {
    ...
    this.checkHideSpan(key, label); //计算input的提示语

    this.changePlaceHolder(label); //计算高度

    this.retop(label.parents(".".concat(FORM_SELECT)));
    this.calcLabelLeft(label, 0, true); //表单默认值

    this.setHidnVal(key, label); //title值

    label.parents(".".concat(FORM_TITLE, " .").concat(NAME)).attr('title', data[key].values.map(function (val) {
      return val.name;
    }).join(','));
  };
  ...
  Common.prototype.initVal = function (id) {
    ...
    $.each(target, function (key, val) {
      var values = val.values,
          div = $("dl[xid=\"".concat(key, "\"]")).parent(),
          label = div.find(".".concat(LABEL)),
          dl = div.find('dl');
      dl.find("dd.".concat(THIS)).removeClass(THIS);
  ...
  Common.prototype.setHidnVal = function (key, label) {
    ...
    label.parents(".".concat(PNAME)).find(".".concat(HIDE_INPUT)).val(data[key].values.map(function (val) {
      return val.value;
    }).join(','));
  };

  Common.prototype.handlerLabel = function (id, dd, isAdd, oval, notOn) {
    var div = $("[xid=\"".concat(id, "\"]")).prev().find(".".concat(LABEL)),
    ...
    if (!notOn) {
      if (on && _instanceof(on, Function) && on(id, vals.concat([]), val, isAdd, dd && dd.hasClass(DISABLED)) == false) {
        return;
      }
    }
    ...
    div.parents(".".concat(FORM_TITLE)).prev().removeClass('layui-form-danger'); //清空搜索值

    fs.config.clearInput && this.clearInput(id);
    this.commonHandler(id, div);
    !notOn && endOn && _instanceof(endOn, Function) && endOn(id, vals.concat([]), val, isAdd, dd && dd.hasClass(DISABLED));
  };

  Common.prototype.addLabel = function (id, div, val) {
    if (!val) return;
    var tips = "fsw=\"".concat(NAME, "\"");
    var _ref = [$("<span ".concat(tips, " value=\"").concat(val.value, "\"><font ").concat(tips, ">").concat(val.name, "</font></span>")), $("<i ".concat(tips, " class=\"xm-iconfont icon-close\"></i>"))],
        $label = _ref[0],
        $close = _ref[1];
    $label.append($close); //如果是radio模式

    var fs = data[id];

    if (fs.config.radio) {
      fs.values.length = 0;
      $("dl[xid=\"".concat(id, "\"]")).find("dd.".concat(THIS, ":not([lay-value=\"").concat(val.value, "\"])")).removeClass(THIS);
  ...
  Common.prototype.delLabel = function (id, div, val) {
    if (!val) return;
    div.find("span[value=\"".concat(val.value, "\"]:first")).remove();
  };

  Common.prototype.checkHideSpan = function (id, div) {
    var parentHeight = div.parents(".".concat(NAME))[0].offsetHeight + 5;
    ...
    var prefix = div.find("span:eq(".concat(count, ")"));
    prefix[0] && prefix.before($("<span style=\"padding-right: 6px;\" fsw=\"".concat(NAME, "\"> + ").concat(div.find('span').length - count, "</span>")));
  };
  ...
  Common.prototype.retop = function (div) {
    ...
    var up = div.hasClass('layui-form-selectup') || dl.css('top').indexOf('-') != -1 || top + dlHeight - 50 > $win.height() && top >= dlHeight;
    div = div.find(".".concat(NAME));
  ...
  Common.prototype.changeShow = function (children, isShow) {
    ...
    var top = children.parents(".".concat(FORM_SELECT)),
        realShow = top.hasClass(FORM_SELECTED),
        id = top.find('dl').attr('xid');
    $(".".concat(PNAME, " .").concat(FORM_SELECT)).not(top).removeClass(FORM_SELECTED);
    ...
    if (isShow) {
      ...
      if(typeof IS_MOBILE==='boolean' && !IS_MOBILE) //手机端不获取焦点
        top.find(".".concat(INPUT)).focus();

      if (!top.find("dl dd[lay-value]:not(.".concat(FORM_SELECT_TIPS, ")")).length) {
        top.find("dl .".concat(FORM_NONE)).addClass(FORM_EMPTY);
      }
    } else {
      top.removeClass(FORM_SELECTED);
      this.clearInput(id);
      top.find("dl .".concat(FORM_EMPTY)).removeClass(FORM_EMPTY);
      top.find("dl dd.".concat(DD_HIDE)).removeClass(DD_HIDE);
      top.find("dl dd.".concat(TEMP)).remove(); //计算ajax数据是否为空, 然后重新请求数据

      if (id && data[id] && data[id].config.isEmpty) {
        this.triggerSearch(top);
      }

      this.changePlaceHolder(top.find(".".concat(LABEL)));
    }

    if (isShow != realShow) {
      var openFun = data[id].config.opened || events.opened[id];
      isShow && openFun && _instanceof(openFun, Function) && openFun(id);
      var closeFun = data[id].config.closed || events.closed[id];
      !isShow && closeFun && _instanceof(closeFun, Function) && closeFun(id);
    }
  };
  ...
  Common.prototype.changePlaceHolder = function (div) {
    //显示于隐藏提示语
    //调整pane模式下的高度
    var title = div.parents(".".concat(FORM_TITLE));
    ...
    var id = div.parents(".".concat(PNAME)).find("dl[xid]").attr('xid');

    if (data[id] && data[id].config.height) {//既然固定高度了, 那就看着办吧
    } else {
      var height = title.find(".".concat(NAME))[0].clientHeight;
      title.css('height', (height > 36 ? height + 4 : height) + 'px'); //如果是layui pane模式, 处理label的高度

      var label = title.parents(".".concat(PNAME)).parent().prev();
    ...
    var input = title.find(".".concat(TDIV, " input")),
        isShow = !div.find('span:last')[0] && !title.find(".".concat(INPUT)).val();
  ...
  Common.prototype.selectAll = function (id, isOn, skipDis) {
    ...
    dl.find("dd[lay-value]:not(.".concat(FORM_SELECT_TIPS, "):not(.").concat(THIS, ")").concat(skipDis ? ':not(.' + DISABLED + ')' : '')).each(function (index, item) {
      item = $(item);

      var val = _this14.getItem(id, item);

      _this14.handlerLabel(id, dl.find("dd[lay-value=\"".concat(val.value, "\"]")), true, val, !isOn);
    });
  };
  ...
  Common.prototype.removeAll = function (id, isOn, skipDis) {
    ...
    if (dl.find('.xm-select-linkage')[0]) {
      ...
      data[id].values.concat([]).forEach(function (item, idx) {
        ...
        do {
          pid = vs[index++];
          li = dl.find(".xm-select-linkage-group".concat(index, ":not(.xm-select-linkage-hide) li[xm-value=\"").concat(pid, "\"]"));
          li.click();
        } while (li.length && pid != undefined);
      });
      return;
    ...
    data[id].values.concat([]).forEach(function (item, index) {
      if (skipDis && dl.find("dd[lay-value=\"".concat(item.value, "\"]")).hasClass(DISABLED)) {} else {
        _this15.handlerLabel(id, dl.find("dd[lay-value=\"".concat(item.value, "\"]")), false, item, !isOn);
      }
    });
  };
  ...
  Common.prototype.reverse = function (id, isOn, skipDis) {
    ...
    dl.find("dd[lay-value]:not(.".concat(FORM_SELECT_TIPS, ")").concat(skipDis ? ':not(.' + DISABLED + ')' : '')).each(function (index, item) {
      item = $(item);

      var val = _this16.getItem(id, item);

      _this16.handlerLabel(id, dl.find("dd[lay-value=\"".concat(val.value, "\"]")), !item.hasClass(THIS), val, !isOn);
    });
  };
  ...
  Common.prototype.skin = function (id) {
    ...
    $("dl[xid=\"".concat(id, "\"]")).parents(".".concat(PNAME)).find(".".concat(FORM_SELECT)).attr('xm-select-skin', skin);
    this.check(id) && this.commonHandler(id, $("dl[xid=\"".concat(id, "\"]")).parents(".".concat(PNAME)).find(".".concat(LABEL)));
  };
  ...
  Common.prototype.onreset = function () {
    ...
    $(document).on('click', '[type=reset]', function (e) {
      $(e.target).parents('form').find(".".concat(PNAME, " dl[xid]")).each(function (index, item) {
        ...
        data[id].config.init.forEach(function (val, idx) {
          if (val && (!temp[val] || data[id].config.repeat) && (dd = dl.find("dd[lay-value=\"".concat(val.value, "\"]")))[0]) {
  ...
  Common.prototype.bindEvent = function (name, id, fun) {
    if (id && _instanceof(id, Function)) {
      fun = id;
      id = null;
    }

    if (fun && _instanceof(fun, Function)) {
      if (!id) {
  ...
  Common.prototype.check = function (id, notAutoRender) {
    if ($("dl[xid=\"".concat(id, "\"]")).length) {
      return true;
    } else if ($("select[xm-select=\"".concat(id, "\"]")).length) {
      if (!notAutoRender) {
        this.render(id, $("select[xm-select=\"".concat(id, "\"]")));
        return true;
      }
    } else {
      delete data[id];
  ...
  Common.prototype.render = function (id, select) {
    common.init(select);
    common.one($("dl[xid=\"".concat(id, "\"]")).parents(".".concat(PNAME)));
    common.initVal(id);
  };
    ...
    if (common.isArray(type)) {
      ...
      type.forEach(function (val, index) {
        if (val && (!temp[val] || fs.config.repeat)) {
          if ((dd = dl.find("dd[lay-value=\"".concat(val, "\"]")))[0]) {
  ...
  Select4.prototype.render = function (id, options) {
    ...
    ($("select[".concat(NAME, "=\"").concat(id, "\"]"))[0] ? $("select[".concat(NAME, "=\"").concat(id, "\"]")) : $("select[".concat(NAME, "]"))).each(function (index, select) {
      var sid = select.getAttribute(NAME);
      common.render(sid, select);
      setTimeout(function () {
        return common.setHidnVal(sid, $("select[xm-select=\"".concat(sid, "\"] + div.").concat(PNAME, " .").concat(LABEL)));
      }, 10);
    });
    return this;
  };
  ...
  Select4.prototype.disabled = function (id) {
    ...
    $.each(target, function (key, val) {
      $("dl[xid=\"".concat(key, "\"]")).prev().addClass(DIS);
    });
    return this;
  };
  ...
  Select4.prototype.undisabled = function (id) {
    ...
    id ? common.check(id) && (target[id] = data[id]) : target = data;
    $.each(target, function (key, val) {
      $("dl[xid=\"".concat(key, "\"]")).prev().removeClass(DIS);
    });
    return this;
  };
  ...
  Select4.prototype.data = function (id, type, config) {
    if (!id || !type || !config) {
      common.log("id: ".concat(id, " param error !!!"));
      return this;
    }

    if (!common.check(id)) {
      common.log("id: ".concat(id, " not render !!!"));
      return this;
    }
  ...
  Select4.prototype.btns = function (id, btns, config) {
    ...
    $.each(target, function (key, val) {
      ...
      var dd = $("dl[xid=\"".concat(key, "\"]")).find(".".concat(FORM_SELECT_TIPS, ":first"));
      ...
      if (btns.length) {
        ...
        var pcInput = dd.parents(".".concat(FORM_SELECT)).find(".".concat(TDIV, " input"));
  ...
  Select4.prototype.search = function (id, val) {
    if (id && common.check(id)) {
      ...
      common.triggerSearch($("dl[xid=\"".concat(id, "\"]")).parents(".".concat(FORM_SELECT)), true);
    }
