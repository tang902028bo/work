...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
    ...
    //主模板
    ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}max-height:{{d.data.height}}px;{{# } }}">'
    ...
    ,Class = function(options){
      ...
      if(that.config.page === false) that.config.limit = 1000;
      that.render();
    };
    ...
    Class.prototype.render = function(){
      ...
      if(!options.no_scrollbar){
        //美化滚动条
        public_scrollbar && public_scrollbar(that.layMain,{theme:'minimal-dark',axis:'yx',callbacks:{
            //table表头、固定列和内容是分开的,当滚动内容时,需要控制表头也滚动(还有个合计行没设置)
            whileScrolling: function(){
              if(this.mcs.direction==='x'){
                that.layHeader.eq(0).scrollLeft(0-this.mcs.left);
                that.layTotal.eq(0).scrollLeft(0-this.mcs.left);
        ...
        that.elem.on('mouseup',function(){
          setTimeout(function(){
            that.layMain.mCustomScrollbar && that.layMain.mCustomScrollbar("update");
          },200);
        })
      }
    };
    ...
    Class.prototype.setColsWidth = function(){
      ...
      - that.layMain.find('table').eq(0).outerWidth();
    ...
    Class.prototype.reload = function(options){
      ...
      if(typeof options === 'function') that.config = options(that.config);
      else that.config = $.extend({}, that.config, options);
      that.render();
    };
    ...
    Class.prototype.pullData = function(curr){
      ...
      if(options.url){ //Ajax请求
        ...
        var ajax_success = function (res) {
          //如果有数据解析的回调，则获得其返回的数据
          if (typeof options.parseData === 'function') {
            res = options.parseData(res) || res;
          }
          
          // == begin.20190214.复制一个parseData方法出来区分,parseDataInit 处理数据格式
          if (typeof options.parseDataInit == "function") {
            res = options.parseDataInit(res);
          }
          // == end
        ...
        var ajax_error = function (errTxt) {
          that.layMain.html('<div class="' + NONE + '">' + errTxt + '</div>');
          that.renderForm();
          that.setColsWidth();
        }
        //发送ajax请求
        var show_loading_dom = that.elem.parent().is(':visible')? that.elem.parent():null;
        public_ajax(options.url,data,ajax_success,show_loading_dom,{
            type: options.method || 'get'
            ,dataType: 'json'
            ,headers: options.headers || {}
            ,error_append:ajax_error
        });
        ...
        layui.each(data, function(i1, item1){
          ...
          that.eachCols(function(i3, item3){
            ...
            var td = ['<td data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
              ...
              //触发编辑的dom ，默认是td ,可设置为图标形式
              + (item3.edit && options.editByIcon ? '<i class="layui-icon layui-icon-edit edit-touch"></i>':'')
              ...
              + function(){
                ...
                return item3.templet ? function(){
                  return typeof item3.templet === 'function' 
                    ? item3.templet(tplData,item3.field)
                    //liucheng20190304 给模板回调添加了一个参数 当前字段名
      ...
      that.eachCols(function(i3, item3){
        ...
        var td = ['<td data-field="'+ field +'" data-key="'+ options.index + '-'+ item3.key +'" '+ function(){
          ...
          ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
            ...
            return item3.totalRow ? '<div class="totalIt" lay-event="'+item3.field+'">'+(parseFloat(totalNums[field]).toFixed(2) || text) +'</div>':'<div class="totalIt" lay-event="'+item3.field+'">'+ text+'</div>';
          }()
        ,'</div></td>'].join('');
    ...
    Class.prototype.scrollPatch = function(){
      var that = this
      ,layMainTable = that.layMain.find('table').eq(0)    //将 that.layMain.children('table') 替换成了 that.layMain.find('table').eq(0),因为滚动条插件破坏了dom,table不是children了
    ...
    Class.prototype.events = function(){
      ...
      //单元格单击事件
      that.layBody.on('click',(that.config.editByIcon? '.edit-touch':'td'), function(e){
        var othis = $(this).closest('td')
        ,field = othis.data('field')
        ,editType = othis.data('edit')
        ,elemCell = othis.children(ELEM_CELL)
        ,index = othis.parents('tr').eq(0).data('index')
        ,rows_data = table.cache[that.key][index]                             //当前行数据
        ,cols_field = options.cols[0][othis.data('key').split('-').pop()];    //当前列表头数据
        
        if(othis.data('off')) return; //不触发事件
        
        //显示编辑表单
        if(editType){
          //编辑时候的值,(复杂类型需要将 id 和 显示值一起传过去,简单类型传入字符串)
          var input_value = {};
          //默认编辑框为 普通输入框
          if(cols_field.field_type === undefined) cols_field.field_type = 1;
          
          input_value[elemCell.text()] =  ( othis.data('content')!==undefined? othis.data('content'):elemCell.text() ) ;

          //编辑框的属性
          var edit_item = public_field_conver_form([{
            placeholder:'请输入内容',
            name:cols_field.title,
            e_name:'edit_value',
            field_type:cols_field.field_type,
            other_set:cols_field.other_set,
            value:input_value
          }]);

          if(edit_item[0].type === "select_table_radio"){
            var data_where_list = othis.closest('.layui-form-item').data('data_where');
            if(data_where_list){
              edit_item[0].data_where = data_where_list[field];
            }
          }
          // //完成编辑后的回调
          // var yes_callback = function(res){
          //   var old_value = table.cache[that.key][index][field];
          //   var submit_fidle = cols_field.field;                //当前修改 名称
          //   var submit_value = res.data.edit_value;             //当前修改 值
          //   var show_html = res.data.edit_value;                //修改后显示的值(需转换)
          //   var form_dom = $(res.form);

            
          //   //转换 table_select 的显示值, (人员选择,部门选择)
          //   if(form_dom.find('.tags_box').length) show_html = submit_value.split(',').map(function(value){ return form_dom.find('.tags[data-id="'+value+'"]').text(); }).join();
          //   //转换 form_select 的显示值, (部门选择)
          //   else if(form_dom.find('.xm-form-select').length) show_html = submit_value.split(',').map(function(value){ return form_dom.find('[fsw=xm-select][value="'+value+'"]').text(); }).join();
          //   //转换 checkbox 的显示值, (暂时没有使用)
          //   else if(form_dom.find('.conver_value').length) show_html = submit_value.split(',').map(function(value){ return form_dom.find('.conver_value[data-id="'+value+'"]').attr('title'); }).join();
          //   //转换 select 的显示值, (select本来是不需要转换的,但是 池的select 需要)
          //   else if(cols_field.field_type === 102 && form_dom.find('.layui-form-select')) show_html = form_dom.find('.layui-select-title input').val();
            
            
          //   table.cache[that.key][index][field] = submit_value;  //更新table缓存中的值
          //   if(cols_field.field_type>=100) table.cache[that.key][index]['chinese_'+field] = show_html; //类型大于99的字段,模板方法需要 chinese 
  
            
          //   //需要显示的值 (如果有模板，则使用模板转换一下)
          //   if(cols_field.templet) show_html = cols_field.templet(table.cache[that.key][index],field);
            
          //   //如果没有产生变化，则不更新, 也不发送到服务器
          //   if(public_decode_html(elemCell.html()) === show_html) return;

          //   //如果设置了回调方法，触发编辑回调
          //   if(that.config.editCallback) that.config.editCallback(submit_fidle,submit_value,rows_data,othis,old_value);

          //   othis.data('content',submit_value);
          //   elemCell.html(show_html);

          //   //如果开启了合计行,并且不是远程数据, 则重载 table (合计行才会重新计算)
          //   if(that.config.totalRow && that.config.url===null) layui.table.reload(that.key);
          // }
          // //显示编辑框
          // public_form('table_edit_box',edit_item,yes_callback,{ table_edit:{ td:othis } });
      ...
      }).on('mouseleave click', 'td', function(){
      ...
      ,gridExpand = function(hide){
        ...
        if(hide){
          setTimeout(function(){
            if(window.layer_this_tips) layer.close(window.layer_this_tips);
          })
          othis.find('.layui-table-grid-down').remove();
        } else if(elemCell.prop('scrollWidth') > elemCell.outerWidth()){
          if(elemCell.find('.'+ ELEM_GRID_DOWN)[0]) return;
          if(options.dont_more_btn){
            layer.tips(elemCell.html(),elemCell,{tips:1,time:0,zIndex:66666666,isOutAnim:false,anim: 5,skin:'table_tips',success:function(layero,index){
              setTimeout(function(){
                window.layer_this_tips = index;
              });
              layero.mouseenter(function(){ delete window.layer_this_tips;
              }).mouseleave(function(){ layer.close(index); })

              var tips_width = layero.width()+layero.outerHeight() / 38 * 18;
              if(tips_width>650) tips_width = 650;
              layero.css({
                'width':tips_width,
                'max-height':'200px',
                'overflow':'auto',
              ...
              if(elemCell.offset().top > layero.offset().top){
                //这里9改成10,鼠标就不能移到弹出框上面了
                layero.css('top',elemCell.offset().top - layero.outerHeight() - 9 );
              }else{
                layero.css('top',layero.offset().top  - 18 );
              }
              
              layero.css('margin-left',0 - (layero.outerWidth() - elemCell.outerWidth())*0.5);
            }});

          }
          else othis.append('<div class="'+ ELEM_GRID_DOWN +'"><i class="layui-icon layui-icon-down"></i></div>');
        }
      };
      ...
      //同步滚动条
      that.layMain.on('scroll', function(){return;
        var othis = $(this)
