...
class Api extends BaseController
{
    ...
    function sync($admin_list=[],$super_admin_list=[]){
        ...
        try{
            ...
            $add_user_array_chunk = array_chunk($this->add_user_array,500);
            $update_user_array_chunk = array_chunk($this->update_user_array,500);
            // dump($this->depart_leader);exit;
            foreach ($add_user_array_chunk as $key => $add_val) {
                Db::name('a_user')->insertAll($add_val);
            }
            foreach ($update_user_array_chunk as $key => $update_val) {
                $UserModel = new AUser();
                $UserModel->saveAll($update_val);
            }

            Db::name("a_department")->where("id",">",0)->update(["leader"=>""]);
            foreach($this->depart_leader as $key => $leader){
                Db::name("a_department")->where("wx_depid",$key)->update(["leader"=>implode(",",array_unique($leader))]);
            }
    ...
    private function sync_dep($dep_count,$dep_id,WxApi $wx_class,$access_token){
        ...
        foreach ($deplist as $dep){
            ...
            if($d_info){
                ...
                db('a_department')->where('id',$dep['id'])->update($data);
            }
            else{
                if($dep_id == $dep['id']){
    ...
    private function  add_update_user($user,$admin_list,$super_admin_list){
        ...
        $id = db('a_user')->where('userid|new_user_id',$user['userid'])->value('id');
        if($id){
            $data["id"] = $id;
